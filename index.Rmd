...
if (!file.exists(image_filename)) {
  image_filename <- "images/placeholder.jpg"
}
```
...
print(sample_stats_x, file = "sample_stats.tex", include.rownames = FALSE, tabular.environment = table_environment, floating = FALSE)
...
print(stats_x, file = "stats.tex", include.rownames = FALSE, tabular.environment = table_environment, floating = FALSE)
...
print(group_stats_x, file = "group_stats.tex", include.rownames = FALSE, tabular.environment = table_environment, floating = FALSE)
...
print(category_stats_x, file = "category_stats.tex", include.rownames = FALSE, tabular.environment = table_environment, floating = FALSE)
...
\begin{figure}[h]
 	\centering
 	\makebox[\textwidth]{\includegraphics[width=\paperwidth]{`r image_filename`}}
...
eDNA sampling was conducted in `r params$site_higherGeography` in `r params$site_date`. A total of `r params$site_num_samples` samples were collected at `r length(na.omit(unique(json$stats$samples$locality)))` locations in the park covering different habitats: `r knitr::combine_words(na.omit(unique(json$stats$samples$locality)))` (Figure \ref{fig:map}). Up to \SI{`r max(na.omit(json$stats$samples$sampleSize))`}{\milli\litre} of seawater was filtered through filter cartridges containing a filter with a \SI{0.8}{\micro\metre} pore size. After sampling, the filter cartridges were flushed with preservation liquid and shipped to UNESCO. From the `r params$site_num_samples` samples, `r length(na.omit(json$stats$samples$locality))` have now (December 2023) been processed. A second batch of samples will be processed by January 2024. DNA from these samples was extracted and amplified, and subsequently sent to the sequencing facility at KU Leuven in Belgium, a specialized university eDNA lab contracted by UNESCO for this work. PCR amplification was done using five primer pairs selected to capture as much biodiversity as possible in the target groups of the study: fish, mammals, and turtles. The results in this preliminary report are based on a first batch of `r length(na.omit(json$stats$samples$locality))` samples that were analyzed covering all locations that were sampled in the World Heritage site. 
...
Sequencing of the DNA from `r length(na.omit(json$stats$samples$locality))` samples resulted in over `r floor(json$stats$dna$reads / 1000000)` million sequence reads (Table \ref{tab:samples}), from which we collected `r format(json$stats$dna$asvs, big.mark = ",")` unique sequences or ASVs. 
\input{stats.tex}
...
The marker sequences obtained from sequencing were matched with sequence reference databases built using public data available from the National Center for Biotechnology Information (NCBI). This resulted in the identification of `r format(nrow(json$species), big.mark = ",")` species, including `r format(sum(na.omit(json$species$group == "fish")), big.mark = ",")` fish species, `r format(sum(na.omit(json$species$group == "mammals")), big.mark = ",")` mammalian species, and `r format(sum(na.omit(json$species$group == "mammals")), big.mark = ",")` turtle species (Table \ref{tab:groups}). Species from other groups, such as `r knitr::combine_words(example_groups)`, were also detected (Figure \ref{fig:tree}). This number of species detected represents about `r found_percentage`% of the `r format(sum(json_full$species$source_gbif | json_full$species$source_obis), big.mark = ",")` species known from `r params$site_higherGeography` in the OBIS database. Of the `r format(nrow(json$species), big.mark = ",")` species detected, `r sum(!(json$species$source_gbif | json$species$source_obis))` are not among the species previously reported from `r params$site_higherGeography` to the UNESCO OBIS database. `r sum(na.omit(json$stats$redlist$edna_species))` of the detected species are listed as threatened on the IUCN Red List (Table \ref{tab:categories}).

A full list of species is added at the end of this report.

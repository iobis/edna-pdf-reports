...
Sequencing of the DNA from `r length(na.omit(json$stats$samples$locality))` samples resulted in over `r floor(json$stats$dna$reads / 1000000)` million sequence reads (Table \ref{tab:samples}), from which we collected `r json$stats$dna$asvs` unique sequences or ASVs. By matching those sequences with reference sequences in public databases, we were able to detect `r nrow(json$species)` species. Most of these species are vertebrates, but also species from other groups were detected, including `r knitr::combine_words(example_groups)` (Figure \ref{fig:tree}). Of the `r nrow(json$species)` species detected, `r sum(!(json$species$source_gbif | json$species$source_obis))` are not among the `r sum(json_full$species$source_gbif | json_full$species$source_obis)` species previously reported from `r params$site_higherGeography` to the UNESCO OBIS database. 

\input{stats.tex}

\input{sample_stats.tex}

```{r message=FALSE, warning=FALSE, fig.pos='p', fig.width=10, fig.height=9, fig.scale=1.2, echo=FALSE, fig.cap='\\label{fig:tree}Number of DNA reads (bubbles) and species detected (bars) by family.', fig.align='center'}
...
The marker sequences obtained from sequencing were matched with sequence reference databases built using public data available from the National Center for Biotechnology Information (NCBI). This resulted in the identification of `r nrow(json$species)` species, of which `r sum(na.omit(json$species$group == "fish"))` fish species, `r sum(na.omit(json$species$group == "mammals"))` mammalian species, and `r sum(na.omit(json$species$group == "mammals"))` turtle species (Table \ref{tab:groups}). This represents about `r found_percentage`% of the species known from `r params$site_higherGeography` in the OBIS database. `r sum(na.omit(json$stats$redlist$edna_species))` of the detected species are listed as threatened on the IUCN Red List (Table \ref{tab:categories}). A full list of species is added at the end of this report.
